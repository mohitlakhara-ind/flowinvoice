...
const statusConfig: Record<string, { label: string; color: string }> = {
  ...
  VIEWED: { label: 'Viewed', color: 'text-indigo-400 bg-indigo-500/10 border-indigo-500/20' },
  ...
  CANCELLED: { label: 'Cancelled', color: 'text-slate-500 bg-slate-500/5 border-slate-500/10' },
}
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-2 px-3 py-2.5 glass glass-hover rounded-xl text-sm text-slate-400 border border-[#2a2a3a] transition-all"
          >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          <div key={s.label} className="glass rounded-xl p-4 border border-[#2a2a3a]">
            <p className="text-xs text-slate-500 mb-1">{s.label}</p>
            <p className="text-lg font-bold text-white">{s.value}{s.unit}</p>
          </div>
        ))}
      </div>
      ...
      {invoices.length === 0 ? (
        <div className="glass rounded-2xl border border-[#2a2a3a] p-20 text-center">
          ...
          <p className="text-sm text-slate-400 mb-6">Create your first invoice and start getting paid.</p>
          <Link
            href="/dashboard/invoices/new"
        ...
        <div className="glass rounded-2xl border border-[#2a2a3a] overflow-hidden">
          <table className="w-full">
            <thead>
              <tr className="border-b border-[#1e1e2e]">
                {['Invoice #', 'Client', 'Date', 'Due', 'Amount', 'Status', ''].map((h) => (
                  <th key={h} className="px-5 py-4 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">
                    {h}
                  </th>
                ))}
              </tr>
              ...
              {invoices.map((invoice) => {
                ...
                return (
                  <tr key={invoice.id} className="hover:bg-white/[0.02] transition-colors group">
                    ...
                    <td className="px-5 py-4 text-sm text-white font-medium">{invoice.client.name}</td>
                    <td className="px-5 py-4 text-sm text-slate-400">
                      {new Date(invoice.issueDate).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: '2-digit' })}
                    </td>
                    <td className={clsx('px-5 py-4 text-sm', isOverdue && invoice.status !== 'PAID' ? 'text-red-400' : 'text-slate-400')}>
                    ...
                    <td className="px-5 py-4">
                      <Link
                        ...
                        className="text-xs text-slate-500 hover:text-indigo-400 transition-colors opacity-0 group-hover:opacity-100"
                      >
                        View →
                      </Link>
                    </td>
