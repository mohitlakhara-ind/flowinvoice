...
const statusConfig: Record<string, { label: string; color: string; dot: string }> = {
  ...
  VIEWED: { label: 'Viewed', color: 'text-indigo-400 bg-indigo-500/10', dot: 'bg-indigo-400' },
  ...
  CANCELLED: { label: 'Cancelled', color: 'text-slate-500 bg-slate-500/5', dot: 'bg-slate-500' },
}

export function RecentInvoices({ invoices }: RecentInvoicesProps) {
  return (
    <div className="glass rounded-2xl border border-[#2a2a3a] h-full flex flex-col">
      ...
      <div className="flex-1 px-4 pb-4 space-y-2">
        {invoices.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 text-center">
            <div className="w-12 h-12 rounded-2xl bg-[#1a1a24] flex items-center justify-center mb-3">
              <span className="text-2xl">📄</span>
            </div>
            <p className="text-sm text-slate-400 font-medium">No invoices yet</p>
            <p className="text-xs text-slate-500 mt-1">Create your first invoice to get started</p>
            <Link
              href="/dashboard/invoices/new"
              className="mt-4 px-4 py-2 text-xs font-medium bg-indigo-600/20 text-indigo-300 rounded-lg hover:bg-indigo-600/30 transition-colors border border-indigo-500/20"
            >
              Create Invoice
            </Link>
          </div>
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-medium text-white truncate">{invoice.client.name}</p>
